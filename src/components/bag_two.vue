    ...
    <div class="bag_two">
        <div class="" v-for="(item, index, key) in data2">
            <header class="bag_list">
                <div class="bagLeft">
                    <p class="bagLef" :class="data2[index].isTrue==0?'':'selected'"  @click="bagChan(index,$event)"></p>
                </div>
                <div class="bagCenter">
...
export default {
    ...
    methods : {
        ...
        //商品列表中是否为选中状态
        bagChan(index,event){
            if(this.data2[index].isTrue == 0){
                event.target.classList.add("selected")
                this.data2[index].isTrue = 1
            }else{
                event.target.classList.remove("selected")
                this.data2[index].isTrue = 0
            }
            // 判断是否选中状态，从而改变全选样式
            for (var i = 0; i < this.data2.length; i++) {
                if(this.data2[i].isTrue == 0){
                    this.isi = 0
                    break
                }else{
                    this.isi = 1
        ...
        changeAll () {
            ...
            if(this.isi == 0){
                for(var i = 0; i < this.data2.length; i++){
                    this.data2[i].isTrue = 1
                }
                this.isi = 1
            }else{
                for(var i = 0; i < this.data2.length; i++){
                    this.data2[i].isTrue = 0
                }
                this.isi = 0
            }
        }
